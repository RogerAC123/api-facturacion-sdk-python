...
class ApiV1CdrConsultarPostRequest(BaseModel):
    """
    ...
    serie: Annotated[str, Field(min_length=1, strict=True, max_length=4)]
    numero: Annotated[str, Field(min_length=1, strict=True, max_length=8)]
    __properties: ClassVar[List[str]] = ["ruc", "tipoDoc", "serie", "numero"]

    @field_validator('tipo_doc')
    def tipo_doc_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['01', '03', '07', '08']):
            raise ValueError("must be one of enum values ('01', '03', '07', '08')")
        return value
